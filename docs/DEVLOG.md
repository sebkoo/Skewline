...
## 2026-08-15 · v0.10 — the noun a review didn't check

**The figure's own boundary line named `0.90` "the margin."** It isn't:
`span.py:459` sets `CANCELLATION_MARGIN = 0.10`, and `:566` compares
`ratio < 1.0 - margin` — `0.10` is the margin, `0.90` is the boundary it
produces. The mistake shipped in a published, README-linked figure and
survived the review that put it there, because that review checked the
numbers against the artifacts and never checked the noun beside them.

- **The comment three lines above the label had the same error, and a grep
  for the wrong words didn't find it.** The first pass searched for the
  label's phrasing; `span_figure.py:260`'s prose — "0.90 is the margin a
  verdict needs to clear" — only turned up once the fix's scope was checked
  by concept instead. A search for wrong words finds wrong words; it doesn't
  find a wrong idea restated in different ones. Both instances are fixed here,
  to `"cancellation boundary"` and `"the boundary a verdict needs to clear"`,
  reusing the noun `span.py:454`, `test_span.py:230` and `:3192` above already
  use rather than inventing new wording.
- **One string, and the figure being generated is why it cost only that.**
  `docs/media/span-light.svg` and `docs/media/span-dark.svg` were
  regenerated with the command `README.md` publishes, not hand-edited; the
  byte comparison in `test_span_figure.py` is what makes that provable, and
  what makes splitting this commit impossible rather than merely unwise —
  stronger than `:1954-1961`'s case, where each half was only inconvenient
  to separate.
- **The gate.** `.venv/bin/python -m unittest discover -s Fit -v` is the
  load-bearing command: green means the regenerated bytes match what's
  committed. The other four are unaffected and were not re-run — no Swift
  file changed, no README or ROADMAP claim changed, and both SVGs' first
